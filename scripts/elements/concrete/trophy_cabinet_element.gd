...
func _build_visual_nodes() -> void:
	...
	# 2x2 multi-cell scaling (200x200 pixels)
	if _bg_rect:
		_bg_rect.size = Vector2(200, 200)
		_bg_rect.position = Vector2(-50, -50) # Anchor at top-left cell center
	if _icon_label:
		_icon_label.size = Vector2(200, 200)
...
func _update_visuals() -> void:
	...
	if _hp_label:
		_hp_label.visible = false # Managed via icon text
	if _bg_rect:
