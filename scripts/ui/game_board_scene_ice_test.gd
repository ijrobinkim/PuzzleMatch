# res://scripts/ui/game_board_scene_ice_test.gd
extends Node2D
...
func _ready() -> void:
	...
	_overlay.restart_requested.connect(func(): GameManager.change_scene("res://scenes/screens/game_board_scene_ice_test.tscn"))
...
func _on_damage_ice_pressed() -> void:
	if _board_view and _board_view.model:
		var damaged_count := 0
		var active_elems: Array = _board_view.model.elements_map.values().duplicate()
		for elem in active_elems:
			if is_instance_valid(elem) and elem.element_id == "snow" and elem.current_health > 0:
				elem.take_damage(1)
				damaged_count += 1
		_update_status("전체 아이스 %d개에 1 데미지 적용!" % damaged_count)
...
func _on_restart_pressed() -> void:
	GameManager.change_scene("res://scenes/screens/game_board_scene_ice_test.tscn")
